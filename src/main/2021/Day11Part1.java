...
public class Day11Part1 {
    ...
    private static final int NOMBRE_TOTAL_ETAPES = 100;

    public static void main(String[] args) {
        int[][] lignes = recupererMatricePieuvres("entree.txt");
        long resultat = calculerNombreFlashes(lignes);
        System.out.println("Réponse=" + resultat);
    }

    private static long calculerNombreFlashes(int[][] matricePieuvresDepart) {
        int[][] matriceActuelle = matricePieuvresDepart;
        long score = 0;
        for (int i = 0; i < NOMBRE_TOTAL_ETAPES; i++) {
            matriceActuelle = faireAvancerDUneEtape(matriceActuelle);
            score += calculerNombrePieuvresQuiViennentDeFlasher(matriceActuelle);
        }
        return score;
    }
