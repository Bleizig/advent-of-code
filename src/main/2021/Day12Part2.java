...
public class Day12Part2 {
    ...
    private static void ajouteSegment(Map<String, List<String>> mapSegments, String etapeDepart, String etapeArrivee) {
        if (peutOnAllerVersCetteEtape(etapeArrivee)) {
    ...
    private static boolean peutOnAllerVersCetteEtape(String etape) {
        return !ETAPE_DEPART.equals(etape);
    }
    ...
    private static List<String> determinerEtapeSuivantesPossibles(Chemin chemin, Map<String, List<String>> mapSegments) {
        ...
        if (ETAPE_FIN.equals(derniereEtape)) {
            return new ArrayList<>();
        }
        List<String> etapesSuivantesPossibles = mapSegments.get(derniereEtape);
        return etapesSuivantesPossibles.stream().filter(etape ->
                estEtapeVisitablePlusieursFois(etape) ||
                        chemin.estEtapeNonVisitee(etape) ||
                        chemin.pasDePetiteGrotteEnDoublon()).collect(Collectors.toList());
    ...
    private static class Chemin {
        ...
        public boolean pasDePetiteGrotteEnDoublon() {
            List<String> etapesPetitesGrottes = etapes.stream().filter(e -> estPetiteGrotte(e) && !ETAPE_DEPART.equals(e) && !ETAPE_FIN.equals(e)).collect(Collectors.toList());
            return etapesPetitesGrottes.size() == etapesPetitesGrottes.stream().distinct().count();
        }

        private boolean estPetiteGrotte(String etape) {
            return etape.toLowerCase().equals(etape);
        }
    }
}
