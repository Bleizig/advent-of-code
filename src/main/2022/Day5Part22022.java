...
public class Day5Part22022 {
    ...
    private static class EtatPiles {
        ...
        private void appliquerMouvement(MouvementsPiles mouvements) {
            Stack<Caisse> caissesDeplacees = new Stack<>();
            //Le -1 est vraiment pourri, la premiere pile est la 1 et non la 0 ...
            for (int i = 0; i < mouvements.nombreCaissesADeplacer; i++) {
                caissesDeplacees.add(getPile(mouvements.pileDepart - 1).pop());
            }
            while (!caissesDeplacees.isEmpty()) {
                getPile(mouvements.pileArrivee - 1).add(caissesDeplacees.pop());
            }
